...
const OutlineWrapper = (props: SBOutlineProps) => {
  ...
  useEffect(() => {
    ...
    console.log("TypeDnD useEffect cards state: " + JSON.stringify(cardsState));

  }, [props]);

  const onClick = useCallback((e: React.MouseEvent<HTMLElement>, text: string) => {
    console.log("TypeDnD onClick useCallback text: " + text);
    if(props.onClick){
  ...
  const dropCard = useCallback((item: {}) => {
    console.log("TypeDnD dropCard useCallback cards state: " + JSON.stringify(cardsStateRef.current));
