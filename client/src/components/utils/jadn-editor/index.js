                        ...
                        case 'symbol':
                            ...
                            switch(token.string){
                                ...
                                case ']':
                                    ...
                                    buffer.markup += ((ind_bool ? newLineBreakAndIndent() : '') + this.newSpan(i, tmp_token, _depth));
                                    break;
                                case ',':
                                    buffer.markup += this.newSpan(i, token, _depth);
            ...
            for (var i = 0; i < buffer2.tokens.length; i++) {
                ...
                switch(token.type) {
                    ...
                    case 'number':
                        ...
                        markup += this.newSpan(i, token, _depth);
                        break;
                    case 'key':
                        indentation += indent() + token.string;
                        markup += indentII() + this.newSpan(i, token, _depth);
                        break;
                    case 'symbol':
                        const islastToken = i === lastIndex
                        switch(token.string) {
                            ...
                            case '}':
                                ...
                                markup += (_adjustmentII + this.newSpan(i, token, _depth));
                                break;
                            case '[':
                            ...
                            case ']':
                                ...
                                if (followsSymbol(buffer2.tokens, i, [']'])) {
                                    if (followedBySymbol(buffer2.tokens, i, [']'])) {
                                        ...
                                        _depth = _depth >= 1 ? _depth - 1 : _depth;
                                        ind_bool = true;
                                    } else if (followedBySymbol(buffer2.tokens, i, [',']) && !followedBySymbol(buffer2.tokens, i, ['['])) {
                                        _depth = _depth >= 1 ? _depth - 1 : _depth;
                                        ind_bool = true;
                                    }
                                }
                                ...
                                markup += ((ind_bool ? indentII() : '') + this.newSpan(i, tmp_token, _depth));
                                break;
                            case ':':
                                indentation += token.string + ' ';
                                markup += this.newSpan(i, token, _depth) + '&nbsp;';
