...
class Command_Generator extends Component {
    ...
    optChange(k, v) {
        ...
        setTimeout(this.setState({ message_update: false }), 3000);
    }

    cmdCreator() {
        let export_records = this.props.schema && this.props.schema.meta ? this.props.schema.meta.exports : []

        let record_def = this.props.schema.hasOwnProperty('types') ? this.props.schema.types.filter(type => type[0] == this.state.command_record) : []
        record_def = (record_def.length == 1 ? record_def[0] : [])

        return (
            <div className='col-md-6'>
		        <h3>actuator.name - Command Creator</h3>
		        ...
		        <div className='card col-12 p-0 mx-auto'>
			        <div className='card-header'>
				        ...
				        <button className='btn btn-primary float-right' onClick={ () => this.makeID() }>Generate ID</button>
			        </div>

			        <Form id='command-fields' className='card-body' onSubmit={ () => { return false; } }>
                        {
    ...
    schema_msg() {
        return (
            <div className='col-md-6'>
		        <h3>Communication: actuator.serialization via actuator.protocol</h3>

		        <ul className='nav nav-tabs'>
			        <li className='nav-item'>
				        <a className='nav-link active show' data-toggle='tab' href='#tab-schema'>Schema</a>
  			        </li>
		        ...
		        <div className='tab-content'>
			        <div className='tab-pane fade active show' id='tab-schema'>
				        <div className='scroll-xl'>
					        <pre id='schema' className='border code'>
					            { FormatJADN(this.props.schema) }
					        </pre>
				        </div>
			        </div>

			        <div className='tab-pane fade position-relative' id='tab-message'>
				        <button className='btn btn-primary position-absolute' style={{ right: 0 }} onClick={ this.sendMsg.bind(this) }>Send</button>
				        <JSONPretty id='message' className='scroll-xl border code' style={{ minHeight: 2.5+'em' }} json={ this.state.message } />
			        </div>
		        </div>
	        </div>
        )
		            ...
		            <div className='modal-dialog h-100 d-flex flex-column justify-content-center my-0' role='document'>
			            <div className='modal-content'>
				            ...
				            <div className='modal-body'>
					            ...
					            <pre className='border code command'></pre>
...
function mapStateToProps(state) {
    return {
        ...
        message: state.Generator.message
    }
}
