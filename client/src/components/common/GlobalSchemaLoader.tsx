...
interface SchemaLoaderProps {
    globalSchema: {file: string, schema: object} | null;
    setGlobalSchema: ({file, schema}: {file: string, schema: object}) => void;
...
const SchemaLoader = (props: SchemaLoaderProps) => {
    ...
    const { globalSchema, setGlobalSchema, 
    ...
    const onFormatClick = (formattedSchema: object) => {
        if (formattedSchema) {
            setGlobalSchema({ file: globalSchema?.file || "", schema: formattedSchema });
        }
    }
    ...
    const sbEditorOnChange = (data: string) => {
        dismissAllToast();
        setIsValid(false);
        setGlobalSchema({ file: globalSchema?.file || "", schema: {data} });
    ...
    const onFileLoad = async (schemaObj?: any, fileStr?: Option) => {
        ...
        if (schemaObj && fileStr) {
            setGlobalSchema({ file: fileStr.label, schema: globalSchema?.schema || {} });
            const fileName = {
                name: getFilenameOnly(fileStr.label),
                ext: getFilenameExt(fileStr.label)
            }
            setFileName(fileName);
            setGlobalSchema({ file: globalSchema?.file || "", schema: schemaObj });
            try {
            ...
            <div className={`${lightBackground ? '' : 'card-header p-2'}`}>
                <div className="row no-gutters">
                    <div className={`${lightBackground ? 'col-lg-10 align-self-center' : 'col-lg-6 align-self-center'}`}>
                        <SBFileLoader
                            opts={schemaOpts}
                            selectedOpt={globalSchema?.file ? { value: globalSchema.file, label: globalSchema.file } : null}
                            loadedFileData={globalSchema?.schema}
                            fileName={fileName}
                            fileExt={schemaFormat?.value}
                            setSelectedFile={(file) => setGlobalSchema({ file: file?.label || "", schema: globalSchema?.schema || {} })}
                    ...
                    <div className={"col text-end align-self-center nowrap"}>
                        {lightBackground ? null : isValidating ? <SBSpinner action={"Validating"} color={"primary"} /> :
                            <SBValidateSchemaBtn
                                ...
                                schemaData={globalSchema?.schema}
                                schemaFormat={schemaFormat?.value}
                                showToast={showToast}
                            />
                        }
                        {showCopy ? <SBCopyToClipboard buttonId='copySchema' data={globalSchema?.schema} customClass='me-1' /> : <></>}
                        {showFormatter ? <SBFormatBtn customClass="me-1" handleFormatClick={onFormatClick} ext={schemaFormat?.value} data={globalSchema?.schema} /> : <></>}
            ...
            { showEditor ? 
                <div className="card-body-page">
                    {isLoading ? <SBSpinner action={'Loading'} isDiv /> :
                        <SBEditor data={globalSchema?.schema || ""} onChange={sbEditorOnChange}></SBEditor>}
                </div>
                :
                <></>
            }
